...
called \texttt{java}. Both of these are part of the \textbf{SDK}, or ``Java
Software Development Kit," that you install in order to program in
Java.\footnote{Just to confuse you, the Java SDK has sometimes been called the
``JDK" (``Java Development Kit") and the ``J2SE" (``Java 2 Standard Edition")
in the past, and you'll likely run across those acronyms as well. To confuse
you even more, the software you need to simply \textit{run} a Java program (as
opposed to writing your own) is called the ``JRE" -- Java Runtime Environment.
