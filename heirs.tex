...
subclass \texttt{B} of another class \texttt{A}, you could have some methods
of \texttt{B} override the existing methods of \texttt{A}, and you could also
have some brand new methods in \texttt{B} that weren't present in \texttt{A}.

Any code that deals with \texttt{A} objects will automatically work for
\texttt{B} objects also, and your new method implementations will be called
when it does. And you can write code that calls your brand new methods, as
long as that code \textit{knows it's dealing with a \texttt{B}}.
